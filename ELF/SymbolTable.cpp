...
template <class ELFT>
bool SymbolTable<ELFT>::reportRemainingUndefines() {
  bool Ret = false;
  for (auto &I : Symtab) {
    Symbol *Sym = I.second;
...
template <class ELFT>
std::vector<Chunk *> SymbolTable<ELFT>::getChunks() {
...
template <class ELFT>
Defined *SymbolTable<ELFT>::find(StringRef Name) {
...
template <class ELFT>
void SymbolTable<ELFT>::dump() {
  for (auto &P : Symtab) {
    Symbol *Ref = P.second;
    if (auto *Body = dyn_cast<Defined>(Ref->Body))
      llvm::dbgs() << Twine::utohexstr(Body->getVA())
                   << " " << Body->getName() << "\n";
  }
}
