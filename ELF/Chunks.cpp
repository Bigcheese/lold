...
template <class ELFT>
void SectionChunk<ELFT>::writeTo(uint8_t *Buf) {
  if (!hasData())
    return;
...
void SectionChunk<ELFT>::applyReloc(uint8_t *Buf, const Elf_Rela *Rel) {
  uint8_t *Off = Buf + FileOff + Rel->r_offset;
  SymbolBody *Body =
      File->getSymbolBody(Rel->getSymbol(File->getObj()->isMips64EL()));
  uint64_t S = cast<Defined>(Body)->getVA();
  uint64_t P = VA + Rel->r_offset;
  switch (Rel->getType(File->getObj()->isMips64EL())) {
  default:
...
template <class ELFT>
bool SectionChunk<ELFT>::hasData() const {
  return Header->sh_type != SHT_NOBITS;
}

template <class ELFT>
uint32_t SectionChunk<ELFT>::getFlags() const {
  return Header->sh_flags;
}

// Prints "Discarded <symbol>" for all external function symbols.
template <class ELFT>
void SectionChunk<ELFT>::printDiscardedMessage() {
  auto Obj = File->getObj();
...
template <class ELFT>
CommonChunk<ELFT>::CommonChunk(const Elf_Sym *S) : Sym(S) {
...
template <class ELFT>
uint32_t CommonChunk<ELFT>::getFlags() const {
  return PF_R | PF_W;
}
